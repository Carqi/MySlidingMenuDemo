...
import com.jouhu.yhestate.core.AppConfig;
...
public class FragmentBottomTabPager extends FragmentActivity 
	...
	private void initView() {
		...
		// 得到fragment的个数
		int count = AppConfig.BOTTOM_MENU.length;
		for (int i = 0; i < count; i++) {
			// 为每一个Tab按钮设置图标、文字和内容
			TabSpec tabSpec = mTabHost.newTabSpec(i + "").setIndicator(i + "");
			// 将Tab按钮添加进Tab选项卡中
			mTabHost.addTab(tabSpec, AppConfig.BOTTOM_MENU[i], null);

			mTabsAdapter.addTab(mTabHost.newTabSpec(i + "")
					.setIndicator(i + ""), AppConfig.BOTTOM_MENU[i], null);
		}
